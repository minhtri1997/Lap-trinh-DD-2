...
public class EditRecipeActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        oldKey = intent.getStringExtra("key");
        oldRecipe = (Recipe) intent.getSerializableExtra("recipe");

        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        materials = oldRecipe.getMaterials();
        guides = oldRecipe.getGuideSteps();
        recipeName = oldRecipe.getName();
        recipeType = oldRecipe.getType();
        note = oldRecipe.getNote();

        fullPhotoUri = Uri.parse(oldRecipe.getImage());
    ...
    private void saveRecipe() {
        databaseReference.child("cooking-recipe").child(oldKey).setValue(recipe, new DatabaseReference.CompletionListener() {
            @Override
            public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                if (databaseError == null) {
                    String key = databaseReference.getKey();
                    saveImage(key);
                    Toast.makeText(EditRecipeActivity.this, "Sửa thành công", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(EditRecipeActivity.this, "Lưu thất bại", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
